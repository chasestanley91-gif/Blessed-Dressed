...
## 2 September 2026 — the Today tab's tasks are now checkable

The Today tab's queue (`#nowq`) and its "This week" list only ever showed as static text — the only checkable list on the page was the separate Checklist tab. Fixed: each queue and this-week item now has its own checkbox, keyed to its `data-s` slug and persisted in a new `todayDone` array alongside the Checklist's existing `done`/`notes` in `cc-state`, saved the same way (self-publish via the `artifact` capability, with a `localStorage` fallback). Checking one strikes it through in place — it does not move the item or delete it, so a person can mark something done between refresh cycles and the automated routine still sees the full context on its next pass. The "Resolved since last update" list stays plain (already-closed items, nothing to check).

Scope note: the shared `ul.tick` bullet style is reused for dozens of plain informational lists across every other tab (Cloth's standing rules, Making's wave plans, Money's customs list, etc.) — only the Today tab's own `#thisweek` list was made checkable; the class itself was left alone everywhere else.

The daily refresh Routine (`trig_012P9msCRKAT6ii2rJ5yPTCE`) was updated to preserve `todayDone` verbatim too, the same as `done`/`notes`, and to keep a stable `data-s` on any queue/this-week item it rewords or adds.
