...
// self-contained (not imported across skill folders) so garment-image-director
// can be installed independently of its siblings (tech-pack-interpreter,
// garment-image-qc) — it only needs to be able to find the repo root and the
// shared pipeline cache, not walk the catalog itself.
...
// Locate the repo root the same way tech-pack-interpreter's catalog.mjs does
// (nearest ancestor containing data-store/options or src/data/options), so
// both skills agree on where .craft-pipeline/ lives without importing across
// skill folders.
