...
const STYLE = {
  ...
  'jacket-lapel': { base: 'worn on a male model over a crisp white dress shirt with NO necktie and nothing at the throat', focus: ['lapel width at its widest point', 'peak/notch/shawl geometry', 'gorge position', 'lapel roll and edge stitch', 'the lower end of the lapel and the exact point at which its edges terminate, completely unobstructed'], crop: 'waist-up, the lapel dominating the frame with the ENTIRE lapel run visible — from the collar down to the exact point where the two front edges meet at the closure, the termination fully inside the frame, never cropped' },
  ...
  // TROUSERS
  'trouser-pocket-depth': { base: 'as a flat-lay macro of the trouser front panel turned OUT to expose the white cotton pocket BAG in full, hanging from the waistband seam, pressed flat; the waistband band retained across the top of the frame as the built-in scale reference', focus: ['the exact vertical DEPTH of the pocket bag from the waistband seam down to the bag bottom', 'the bag bottom curve fully in frame', 'the waistband band across the top of the frame as the scale anchor'], crop: 'a tight vertical macro with the full pocket bag from waistband seam to bag bottom the entire subject, the waistband visible across the top; identical camera distance for every member of this depth family' },
